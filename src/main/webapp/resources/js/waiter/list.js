...
function initData(pageNO,pageSize,userName){
    var totalPage = 1;
    $.ajax({
        type : "POST",
        url : getRootPath()+"/web/waiter/getWaiter",
        data :{
            name:userName,
            pageNO:pageNO,
            pageSize:pageSize
...
function buildData(dataList){
    ...
    if(dataList.length == 0){
        ...
        for (var i=0; i<len; i++){
            ...
            var timeConfigArr = JSON.stringify(obj.waiterTimeConfigs);
                str+="<tr>" +
                "<td>"+obj.name+"</td>" +
                "<td>"+gender+"</td>" +
                "<td>"+createTime+"</td>" +
 ...
 * @constructor
 */
function Scheduling(waiterId,timeConfigArr){
    ...
    buildCheckBok(timeConfig,waiterId,timeConfigArr);
}
 ...
 * @param dataArr
 */
function  buildCheckBok(dataArr,waiterId,timeConfigArr){

    var str_ = "";
    for (var i=0; i<dataArr.length; i++){
        var checked = "";
        if(null != timeConfigArr){
        ...
        buttons: {
            ...
            success: {
                ...
                callback: function () {
                    ...
                    $.ajax({
                        type : "POST",
                        url : getRootPath()+"/web/waiter/setTimeConfig",
                        data : {
                            waiterId:waiterId,
                            times:times
                        },
        ...
        buttons: {
            ...
            success: {
                ...
                callback: function () {
                    $.ajax({
                        type : "POST",
                        url : getRootPath()+"/web/waiter/updateWaiter",
                        data : {
                            id:id,
                            isValid:value
                        },
